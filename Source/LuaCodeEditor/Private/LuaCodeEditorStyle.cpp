...
TSharedPtr< FSlateStyleSet > FLuaCodeEditorStyle::StyleSet = nullptr;
...
void FLuaCodeEditorStyle::Initialize()
{
	// Only register once
	if( StyleSet.IsValid() )
	{
	...
	StyleSet = MakeShareable(new FSlateStyleSet("LuaCodeEditor") );

	StyleSet->SetContentRoot(FPaths::ProjectPluginsDir() / TEXT("LuaContentBrowser/Resources"));
...
void FLuaCodeEditorStyle::Shutdown()
{
	if( StyleSet.IsValid() )
	{
...
const ISlateStyle& FLuaCodeEditorStyle::Get()
{
	return *( StyleSet.Get() );
}

const FName& FLuaCodeEditorStyle::GetStyleSetName()
{
